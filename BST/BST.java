...
public class BST<Key extends Comparable<Key>, Value> {
     ...
     * Tree height.
     *
     * Asymptotic worst-case running time using Theta notation: N - This is because the function has to look through every node and subtree in order to find out which height is the largest.
     *
    ...
    private Key median(Node x, int position) {
    	int leftSubSize = size(x.left);
    	if(leftSubSize > position) {
    		return median(x.left, position);
    	}
    	else if(leftSubSize < position) {
    		return median(x.right, (position - leftSubSize -1));
    	}
    	else {
    		return x.key;
    	}
    ...
    private String printKeysInOrder(Node node) { 
    	...
    	if (node == null) {
    		...
    		result += "(" + printKeysInOrder(node.left);
    		result += node.key;
    		result += printKeysInOrder(node.right) + ")"; 
    		return result;
    	}
    }
